...
it('can add the star name and star symbol properly', async() => {
    ...
    // then
    assert.equal(await starNotaryContract.symbol(), 'CMS');
    assert.equal(await starNotaryContract.name(), 'CosmosStar');
});
...
it('lets a user transfer a star', async() => {
    ...
    // given
    const starId = createNewStarId();
    await starNotaryContract.createStar('The ?th Star!', starId, {from: user1});
